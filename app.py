    ...
    if request.method == "POST":
        ...
        if arquivo:
            caminho_original = os.path.join(
                app.config["UPLOAD_FOLDER"], "temp_original.jpg"
            )
            ...
            dados_analise = {
                ...
                "confianca": (
                    f"{result.caption.confidence * 100:.2f}%"
                    if result.caption
                    else "0%"
                ),
                "tags": (
                    [tag.name for tag in result.tags.list[:5]] if result.tags else []
                ),
            }
            ...
            caminho_resultado = os.path.join(
                app.config["UPLOAD_FOLDER"], nome_resultado
            )
            ...
            return render_template(
                "index.html",
                imagem_exibir=caminho_resultado,
